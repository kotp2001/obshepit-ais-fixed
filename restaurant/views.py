...
def log_action(request, action, description=''):
    try:
        ...
        ActionLog.objects.create(user=user, action=action, description=description, ip_address=get_client_ip(request))
    except Exception:
        pass
...
def generate_receipt_pdf(order):
    try:
        ...
        items_text = '\n'.join(f"  {item.dish.name} x{item.quantity} = {float(item.price * item.quantity):.2f} руб." for item in order.items.all())
    ...
    try:
        ...
        try:
            ...
            if attempt.blocked_until and datetime.now() < attempt.blocked_until:
                ...
                return JsonResponse({'success': False, 'error': f'Слишком много попыток. Попробуйте через {remaining} мин.', 'blocked': True})
        ...
        if user:
            ...
            if attempt:
                ...
                attempt.save()
            role = 'admin' if user.is_superuser else (user.profile.role if hasattr(user, 'profile') and user.profile.role else 'staff')
            log_action(request, 'login', f'Вход: {username} (роль: {role})')
            return JsonResponse({'success': True, 'role': role, 'username': user.username})
        else:
            MAX_ATTEMPTS, BLOCK_MINUTES = 5, 15
            if attempt:
                attempt.attempts += 1
            ...
            remaining = MAX_ATTEMPTS - (attempt.attempts if attempt else 1)
            msg = f'Неверный логин или пароль. Осталось попыток: {remaining}' if remaining > 0 else 'Неверный логин или пароль.'
...
def api_dishes(request):
    ...
    data = [{'id': d.id, 'name': d.name, 'description': d.description, 'price': float(d.price), 'category': d.category.name, 'category_id': d.category.id} for d in dishes]
...
def api_create_order(request):
    try:
        ...
        current_time = parse_datetime(client_time) if client_time else datetime.now()
        order = Order(table=table, status='new', guest_count=guest_count, created_at=current_time)
        order.save()
        total = Decimal('0')
        for item in items:
            dish = Dish.objects.get(id=item['dish_id'])
            OrderItem.objects.create(order=order, dish=dish, quantity=item['quantity'], price=dish.price, status='pending')
            total += dish.price * item['quantity']
        order.total_amount = total
        order.save()
        table.status = 'occupied'
        table.save()
        log_action(request, 'create_order', f'Заказ #{order.id}, стол {table.number}, сумма {float(total):.2f}')
...
def api_active_orders(request):
    ...
    data = []
    for order in orders:
        items = [{'id': i.id, 'dish_name': i.dish.name, 'quantity': i.quantity, 'status': i.status} for i in order.items.all()]
        waiter_name = order.waiter.first_name or order.waiter.username if order.waiter else 'Не указан'
        data.append({
            'id': order.id, 'table_number': order.table.number,
            'created_at': order.created_at.strftime('%H:%M') if order.created_at else '',
            'status': order.status, 'items': items, 'waiter_name': waiter_name
        })
    return JsonResponse({'success': True, 'data': data})
...
def api_update_item_status(request):
    try:
        ...
        order.save()
        log_action(request, 'update_item', f'Блюдо {item.dish.name} → {item.status}, заказ #{order.id}')
...
def api_pay_fixed(request):
    if request.method != 'POST':
        return JsonResponse({'success': False, 'error': 'Only POST'}, status=405)
    try:
...
def api_order_receipt(request, order_id):
    try:
        order = Order.objects.get(id=order_id)
        items = [{'name': i.dish.name, 'quantity': i.quantity, 'price': float(i.price), 'total': float(i.price * i.quantity)} for i in order.items.all()]
        payment_label = {'cash': 'Наличные', 'card': 'Карта', 'qr': 'QR-код'}.get(order.payment_method, 'Не оплачен')
        receipt_url = f'/receipts/{order.receipt.id}/download/' if hasattr(order, 'receipt') and order.receipt else None
        return JsonResponse({'success': True, 'data': {
            'order_id': order.id, 'table_number': order.table.number,
            'created_at': order.created_at.strftime('%d.%m.%Y %H:%M') if order.created_at else '',
            'items': items, 'total': float(order.total_amount), 'payment_method': payment_label, 'receipt_url': receipt_url
        }})
    except Exception as e:
        return JsonResponse({'success': False, 'error': str(e)}, status=400)
    ...
    try:
        ...
        if receipt.pdf_file:
            ...
            if os.path.exists(fpath):
                return FileResponse(open(fpath, 'rb'), as_attachment=True, filename=f'receipt_{receipt.order.id}.txt')
...
def api_reports(request):
    ...
    if period == 'day': start_date = end_date = today
    elif period == 'month': start_date = today - timedelta(days=30); end_date = today
    elif period == 'custom':
    ...
    else: start_date = today - timedelta(days=7); end_date = today
    orders = Order.objects.filter(status='paid', created_at__date__gte=start_date, created_at__date__lte=end_date).prefetch_related('items__dish')
    ...
    for order in orders:
        for item in order.items.all():
            n = item.dish.name
            if n not in dish_data: dish_data[n] = {'count': 0, 'price': float(item.price)}
            dish_data[n]['count'] += item.quantity
    popular = [{'name': n, 'count': d['count'], 'price': d['price']} for n, d in sorted(dish_data.items(), key=lambda x: -x[1]['count'])[:5]]
    daily = [{'date': (start_date + timedelta(days=i)).strftime('%d.%m'),
              'revenue': sum(float(o.total_amount) for o in orders.filter(created_at__date=start_date + timedelta(days=i))),
              'orders': orders.filter(created_at__date=start_date + timedelta(days=i)).count()} for i in range((end_date - start_date).days + 1)]
    log_action(request, 'view_report', f'Период: {period}')
    return JsonResponse({'success': True, 'data': {'total_revenue': total_revenue, 'total_orders': total_orders, 'avg_check': avg_check, 'popular_dishes': popular, 'daily_data': daily}})

@require_http_methods(["GET"])
def api_maintenance_logs(request):
    logs = MaintenanceLog.objects.all().order_by('-date')
    data = [{'id': l.id, 'date': l.date.strftime('%Y-%m-%d'), 'work_performed': l.work_performed, 'performed_by': l.performed_by, 'signature': l.signature} for l in logs]
    return JsonResponse({'success': True, 'data': data})
...
def api_maintenance_logs_add(request):
    try:
        body = json.loads(request.body)
        log = MaintenanceLog.objects.create(date=body.get('date'), work_performed=body.get('work_performed'), performed_by=body.get('performed_by'), signature=body.get('signature', ''))
...
def api_action_logs(request):
    ...
    data = [{'id': l.id, 'user': l.user.username if l.user else 'Аноним', 'action': l.get_action_display(), 'description': l.description, 'ip': l.ip_address or '', 'timestamp': l.timestamp.strftime('%d.%m.%Y %H:%M:%S')} for l in logs]
    return JsonResponse({'success': True, 'data': data})
...
def auto_backup_trigger(request):
    ...
    try:
        ...
        if not db.get('HOST'): return JsonResponse({'success': False, 'error': 'PostgreSQL не настроен'})
        ...
        backup_file = os.path.join(backup_dir, f'auto_backup_{ts}.sql')
        env = os.environ.copy()
        env['PGPASSWORD'] = db.get('PASSWORD', '')
        subprocess.run(['pg_dump', '-h', db['HOST'], '-p', str(db.get('PORT') or '5432'), '-U', db['USER'], '-d', db['NAME'], '-f', backup_file, '--no-password'], env=env, timeout=60, check=True)
        with open(backup_file, 'r', encoding='utf-8', errors='replace') as f:
            lines = f.readlines()
        with open(backup_file, 'w', encoding='utf-8') as f:
            f.writelines(l for l in lines if not l.startswith('\\'))
        log_action(request, 'create_backup', f'Авто-бэкап: auto_backup_{ts}.sql')
        return JsonResponse({'success': True, 'file': f'auto_backup_{ts}.sql'})
    except Exception as e:
        return JsonResponse({'success': False, 'error': str(e)})
...
def api_change_password(request):
    try:
        ...
        if len(new_pass) < 6:
            return JsonResponse({'success': False, 'error': 'Пароль не менее 6 символов'})
...
def api_blocked_users(request):
    ...
    data = [{'username': b.username, 'attempts': b.attempts, 'blocked_until': b.blocked_until.strftime('%H:%M:%S') if b.blocked_until else '', 'ip': b.ip_address or ''} for b in blocked]
    return JsonResponse({'success': True, 'data': data})

@login_required(login_url="/")
def admin_backup(request):
    if not (request.user.is_staff or request.user.is_superuser): return redirect('/')
    backup_dir = '/tmp/backups'
    os.makedirs(backup_dir, exist_ok=True)
    if request.method == 'POST':
        try:
            from django.conf import settings
            db = settings.DATABASES['default']
            if not db.get('HOST'): raise Exception('Настройки БД не найдены')
            ts = datetime.now().strftime('%Y%m%d_%H%M%S')
            backup_file = os.path.join(backup_dir, f'backup_{ts}.sql')
            env = os.environ.copy()
            env['PGPASSWORD'] = db.get('PASSWORD', '')
            subprocess.run(['pg_dump', '-h', db['HOST'], '-p', str(db.get('PORT') or '5432'), '-U', db['USER'], '-d', db['NAME'], '-f', backup_file, '--no-password'], env=env, timeout=60, check=True)
            with open(backup_file, 'r', encoding='utf-8', errors='replace') as f:
            ...
            with open(backup_file, 'w', encoding='utf-8') as f:
                f.writelines(l for l in lines if not l.startswith('\\'))
            log_action(request, 'create_backup', f'backup_{ts}.sql')
            return JsonResponse({'success': True, 'message': f'Резервная копия создана: backup_{ts}.sql'})
        except Exception as e:
            return JsonResponse({'success': False, 'error': str(e)})
    backups = []
    for fname in sorted(os.listdir(backup_dir), reverse=True):
        if fname.endswith('.sql'):
            fpath = os.path.join(backup_dir, fname)
            stat = os.stat(fpath)
            backups.append({'name': fname, 'date': datetime.fromtimestamp(stat.st_mtime).strftime('%d.%m.%Y %H:%M'), 'size': round(stat.st_size / 1024, 1)})
    return render(request, 'backup.html', {'backups': backups})

@login_required(login_url="/")
def admin_backup_download(request, filename):
    safe = os.path.basename(filename)
    path = os.path.join('/tmp/backups', safe)
...
def admin_backup_restore(request, filename):
    ...
    try:
        ...
        env = os.environ.copy()
        env['PGPASSWORD'] = db.get('PASSWORD', '')
        subprocess.run(['psql', '-h', db['HOST'], '-p', str(db.get('PORT') or '5432'), '-U', db['USER'], '-d', db['NAME'], '-f', path, '--no-password'], env=env, timeout=120, check=True)
        return redirect('/backup/')
    except Exception as e:
